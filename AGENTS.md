...
Backlog is trimmed from the front. For full-screen TUIs (Gemini CLI, Claude Code) that discards the screen-setup sequence first, so replaying later frames to a fresh xterm paints garbage on a screen that was never established → black terminal that still accepts keystrokes. The stream announces truncation; the client resets and requests a repaint. pty-manager early-returns on unchanged geometry, so a repaint must nudge cols by one and back to force a real SIGWINCH.
